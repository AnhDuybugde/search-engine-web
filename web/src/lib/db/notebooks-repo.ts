...
async function loadChunksUncached(
  ...
  if (process.env.DATABASE_URL) {
    try {
      ...
      try {
        rows = options.includeEmbeddings === false
          ? await db
              .select({
                id: chunks.id,
        ...
        if (includeEmbeddings && isMissingEmbeddingColumn(dbErr)) {
      ...
      console.warn("Drizzle direct load chunks failed, falling back to Supabase REST:", err);
    }
  }
