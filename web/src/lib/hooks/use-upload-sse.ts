...
/** Raw store pipeline: receive → extract → store (no chunk/embed). */
    ...
    try {
      ...
      if (ct.includes("application/json")) {
        ...
        setState({
          ...
          error: null,
          logs: [
            `Stored raw source ${data.title} (${data.charCount ?? 0} chars, 0 chunks)`,
          ],
...
function applyUploadEvent(
  ...
  switch (event.type) {
    ...
    case "upload_completed":
      logs.push(
        `Upload complete · raw source · ${event.source.charCount} chars · 0 chunks · total ${event.timing.totalMs ?? "?"}ms`,
      );
      steps.receive = "success";
      steps.extract = "success";
      steps.store = "success";
